...
public interface ProductOptionValueRepository extends JpaRepository<ProductOptionValue, Long> {
	...
	@Query("select p from ProductOptionValue p join fetch p.merchantStore pm left join fetch p.descriptions pd where pm.id = ?1 and (pd.name like concat('%', ?2, '%') or p.code like concat('%', ?2, '%')) and pd.language.id = ?3")
