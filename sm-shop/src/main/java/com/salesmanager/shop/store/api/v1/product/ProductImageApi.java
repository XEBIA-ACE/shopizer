...
public class ProductImageApi {
	...
	@Operation(summary = "Get images for a given product")
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "List of ProductImage found", content = @Content(schema = @Schema(implementation = List.class))) })
	@ResponseBody
	@Parameters({ @Parameter(name = "store", in = ParameterIn.QUERY, schema = @Schema(type = "string", defaultValue = "DEFAULT")),
			@Parameter(name = "lang", in = ParameterIn.QUERY, schema = @Schema(type = "string", defaultValue = "en")) })
